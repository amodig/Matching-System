...
class RelatedArticlesHandler(MainBaseHandler):
    ...
    def post(self):
        ...
        iterator = mm.fetch()
        message = {
            "related_articles": [{"id": data_row[0], "distance": display_distance[element], "title":  data_row[1], "abstract": data_row[2],
            "author": data_row[3], } for data_row, element in zip(iterator, sorted_index[::-1][:10])]
        }
        print message
        self.json_ok(message)
