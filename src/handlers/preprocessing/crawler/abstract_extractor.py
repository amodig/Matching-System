...
class AbstractExtractor():
    ...
    @staticmethod
    def pdf_abstract_extractor(pdf_path):
        def __pdf_to_txt(path):
            null_f = open(devnull, "w")
            contents = Popen(["ps2ascii", path], stdout=PIPE, stderr=null_f).communicate()[0]
            null_f.close()
        ...
        try:
            print pdf_path
            contents = __pdf_to_txt(pdf_path)
        except Exception, e:
            stderr.write("Debug: path, %s" % pdf_path)
