...
def parse_doc_list(docs, vocab):
    ...
    # if there is not one document then append it to a list
    if (type(docs).__name__ == 'str'):
        temp = list()
        temp.append(docs)
        docs = temp

    # initialize the length of docs list, words ID list and word count times list
    D = len(docs)
    wordids = list()
    wordcts = list()

    # give each different keyword ID and count how many times they appear in one document.
    for d in range(0, D):
        # the document has been transformed into csv format
        separator = ','
        words = docs[d].split(separator)
        ddict = dict()
        for word in words:
            if (word in vocab):
