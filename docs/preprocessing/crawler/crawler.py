    ...
    def _parse_and_store(self, soup, foreign_key):
        ...
        for h in soup.findAll('h3', {'class': 'gs_rt'}):
            try:
                ...
                sql = u"INSERT INTO PaperLinks (Link_ID, PaperLink, Paper_ID) VALUES ( default,\"" + link + u"\", " +\
                      str(foreign_key) + u")"
                print sql
                self.mm.execute_sql(sql, log_file)
            except Exception, e:
...
class CourseContentCrawler(AbstractCrawler):
    ...
    def _parse_and_store(self, soup):
        ...
        for iter in soup.findAll('span', {'class': 'views-field-title'}):
            ...
            sql = u"INSERT INTO CourseDescription (Course_ID, Course_Name, Course_Description) VALUES ( default,\"" +\
                  course_name + u"\", \"" + course_description + u"\")"
            print sql  
            self.mm.execute_sql(sql, log_file)
